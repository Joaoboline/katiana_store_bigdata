...
if all(col in raw.columns for col in ["qtd", "preco"]):
    raw["valor_total"] = raw["qtd"] * raw["preco"]
    if "desconto" in raw.columns:
        raw["valor_total"] = raw["valor_total"] - raw["desconto"].fillna(0)
else:
    raise ValueError("As colunas 'qtd' e/ou 'preco' não foram encontradas na planilha.")

MARGEM_LUCRO = 0.30
raw["Lucro (R$)"] = raw["valor_total"] * MARGEM_LUCRO
raw["Capital Investido (R$)"] = raw["valor_total"] - raw["Lucro (R$)"]

print("✅ Colunas 'valor_total', 'Lucro (R$)' e 'Capital Investido (R$)' calculadas com sucesso.")
...
df_diario = (
    raw.groupby("data")[["valor_total", "Lucro (R$)", "Capital Investido (R$)"]]
    .sum()
    .reset_index()
)
...
forecast_df["Limite Inferior (R$)"] = forecast_df["Previsão (R$)"].apply(
    lambda x: max(x * 0.85, 0)
)
forecast_df["Limite Superior (R$)"] = forecast_df["Previsão (R$)"].apply(
    lambda x: min(x * 1.15, media * 2)
)
...
ws["A1"] = "📊 Dashboard Katiana Store — Vendas, Lucro e Investimento"
